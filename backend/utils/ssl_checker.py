...
def _tls_target(domain, port):
    authority = f"[{domain}]" if ":" in domain and not domain.startswith("[") else domain
    return validate_scoped_target(f"https://{authority}:{port}")
...
def get_ssl_certificate(domain: str, port: int = 443) -> dict:
    ...
    with open_pinned_connection(_tls_target(domain, port), timeout=3) as sock:
...
def check_tls_versions(domain: str, port: int = 443) -> list:
    ...
    for label, tls_version in versions_to_test:
        try:
            ...
            with open_pinned_connection(_tls_target(domain, port), timeout=2) as sock:
...
def run_ssl_check(domain: str, use_mock: bool = False, strict_live: bool = False) -> SslResult:
    ...
    if use_mock and strict:
        raise RuntimeError("STRICT_LIVE_MODE: Mock SSL check execution is disabled in strict live mode.")

    if use_mock:
        return _load_mock()

    try:
        from utils.target_policy import validate_target, TargetPolicyError
        validated = normalize_target(domain if "://" in domain else f"https://{domain}")
        hostname, port = validated.host, validated.port
        cert_data = get_ssl_certificate(hostname, port)
        ...
        return SslResult(valid=False, status="warning", error=str(exc))
    except (ssl.SSLError, socket.timeout, ConnectionRefusedError, OSError) as exc:
        if strict:
            raise RuntimeError(f"STRICT_LIVE_MODE: SSL check failed for {domain}: {exc}")
        return SslResult(
            valid=False,
            status="error",
            error=str(exc),
