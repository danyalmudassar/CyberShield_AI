...
BEARER_KEY_REGEX = re.compile(r'(Bearer\s+|(?:api_key|apikey|access_key|secret_key|auth_token)=|[?&]key=)[A-Za-z0-9._-]+', re.IGNORECASE)
...
def sanitize_secrets_only(text: str) -> str:
    """Sanitize raw text by replacing secrets/API keys with safe placeholders, preserving domains and IPs."""
    if not text:
        return ""
    return BEARER_KEY_REGEX.sub(rf'\1{REDACTED_SECRET}', text)
