...
def call_llm_json(
    ...
    raw_response = call_llm(prompt, system_prompt=system_prompt, use_mock=use_mock, timeout=timeout)
    if not raw_response:
        return _load_mock_json()
    ...
    try:
        ...
        logger.warning("Failed to parse valid JSON payload from LLM response (%s). Utilizing fallback mock payload.", str(err)[:100])
        return _load_mock_json()
...
def call_llm(
    ...
    llm_model = os.getenv("LLM_MODEL", os.getenv("QWEN_MODEL", "gemma4:31b-cloud"))
