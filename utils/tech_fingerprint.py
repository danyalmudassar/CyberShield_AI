...
def check_robots_txt(domain: str) -> str:
    ...
    resp, _ = safe_request(f"https://{domain}/robots.txt", timeout=(5, 15))
    if resp is not None and resp.status_code == 200:
        return resp.text
    resp, _ = safe_request(f"http://{domain}/robots.txt", timeout=(5, 15))
    if resp is not None and resp.status_code == 200:
        return resp.text
    return None
...
def check_sitemap(domain: str) -> bool:
    ...
    resp, _ = safe_request(f"https://{domain}/sitemap.xml", timeout=(5, 15))
    if resp is not None and resp.status_code == 200 and "<" in resp.text[:500]:
        return True
    resp, _ = safe_request(f"http://{domain}/sitemap.xml", timeout=(5, 15))
    if resp is not None and resp.status_code == 200 and "<" in resp.text[:500]:
        return True
    return False
...
def run_tech_fingerprint(domain: str, use_mock: bool = False) -> TechResult:
    ...
    try:
        ...
        html = ""
        headers = {}
        resp, err = safe_request(f"https://{domain}", timeout=(5, 15))
        if resp is not None:
            html = resp.text
        else:
            resp, err = safe_request(f"http://{domain}", timeout=(5, 15))
            if resp is not None:
                html = resp.text

        # Grab headers separately
        try:
            headers = get_headers(f"https://{domain}", timeout=(5, 15))
        except Exception:
            try:
                headers = get_headers(f"http://{domain}", timeout=(5, 15))
            except Exception:
                headers = {}
