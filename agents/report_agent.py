...
Two output modes:
  ...
  2. AI-enhanced (optional): Qwen-Max prose executive summary
...
from models import ReportData, Finding, PisfControl, PisfResult
...
def _build_executive_summary(domain: str, findings: list, pisf_result: PisfResult,
    ...
    """Build executive summary — AI-enhanced if available, else template."""
    if ai_analysis and isinstance(ai_analysis, dict):
        ai_exec = ai_analysis.get("executive_summary", "")
        if ai_exec and len(ai_exec.strip()) > 20:
            # Replace default mock domain name with actual target domain if present
            ai_exec = ai_exec.replace("testphp.vulnweb.com", domain)
            return ai_exec.strip()
    ...
    summary = (
        ...
        f"PISF 2026 compliance score: {score:.0f}% ({pass_count} of {assessable_count} assessable controls compliant; {non_assessable_count} controls marked NOT_ASSESSABLE due to scope boundaries). "
...
def _build_scope_methodology(domain: str, scope_type: str, timestamp: str) -> str:
    ...
    scope_label = "Passive Reconnaissance Only" if scope_type == "passive_only" else "Full Penetration Test"
...
def _calculate_security_score(findings: list, pisf_result: PisfResult) -> float:
...
def _generate_pdf(report_data: ReportData, domain: str) -> str:
    ...
    pdf.cell(0, 8, _safe_ascii(f"Security Score: {report_data.security_posture_score:.1f}/100"), new_x="LMARGIN", new_y="NEXT", align="C")
    pdf.ln(30)
...
def generate_report(scan_data: dict = None, use_mock: bool = False) -> dict:
    ...
    Args:
        ...
        use_mock: if True or scan_data is None, use built-in mock data.

    Returns:
        dict with report (ReportData), pdf_path, security_score, findings_summary, status.
    """
    if use_mock or scan_data is None:
        ...
        # Try to load mock Qwen for executive summary
        try:
        ...
        findings = []
        findings.extend(scan_data.get("all_findings", []))
        findings.extend(scan_data.get("findings", []))
        findings.extend(scan_data.get("pentest_findings", []))
        if scan_data.get("recon_result") and isinstance(scan_data["recon_result"], dict):
            findings.extend(scan_data["recon_result"].get("findings", []))
    ...
    if isinstance(scan_data, dict):
        ...
        fallback_triggered = fallback_triggered or fb_direct or fb_ai or fb_threat or fb_visual or fb_pentest or fb_finding
